...
const StatsSlide = ({ content, isStatic = false }) => {
    ...
    const StatItem = ({ stat, index }) => {
        ...
        return (
            ...
            >
                ...
                {/* 巨大なバリュー（数値） */}
                <div style={{
                    fontSize: activeStats.length > 2 ? '4.2cqi' : '5.2cqi', // さらに少し縮小
                    fontWeight: 900,
                    color: 'var(--slide-heading)',
                    lineHeight: 1.0, // 行間を詰め
                    marginBottom: '0.5cqi', // 余白を大幅に削減
    ...
    return (
        <div className="json-slide-layout indigo-style h-full flex flex-col">
            ...
            {/* メインレイアウト */}
            <div className="flex-1 flex mt-[0.5cqi] overflow-hidden">
                {isTwoColumn ? (
                    // 2カラム (左：示唆 / 右：指標グリッド)
                    <div className="w-full flex items-center gap-[3cqi]">
                        {mainText && (
                            <div className="flex-[4] flex flex-col pr-[1cqi]">
                                <h3 style={{ fontSize: '1.1cqi', color: '#94A3B8', fontWeight: 800, letterSpacing: '0.1em', marginBottom: '1cqi', textTransform: 'uppercase' }}>
                                    Key Takeaway
                                </h3>
                                <motion.div
                                    style={{ borderLeft: '3px solid var(--slide-primary)', paddingLeft: '1.5cqi' }}
                                    initial={!isStatic ? { opacity: 0, x: -15 } : {}}
                                    animate={{ opacity: 1, x: 0 }}
                                    transition={{ duration: 0.5, delay: 0.2 }}
                                >
                                    <div style={{ fontSize: activeStats.length > 3 ? '1.3cqi' : '1.4cqi', color: 'var(--slide-body)', lineHeight: 1.6, fontWeight: 500 }}>
                        ...
                        {/* 中央ディバイダー */}
                        <div className="w-[1px] h-[70%] bg-slate-200" />

                        <div className="flex-[7] flex flex-col justify-center pl-[1cqi]">
                            <div className={`grid ${gridClass} gap-y-[3cqi] gap-x-[1cqi]`}>
                    ...
                    // 1カラム (上：指標グリッド / 下：示唆)
                    <div className="w-full flex flex-col justify-center gap-[1.5cqi]">
                        <div className="w-full">
                            <div className={`grid ${gridClass} gap-y-[1.5cqi] gap-x-[1cqi]`}>
                                {activeStats.map((stat, i) => <StatItem key={i} stat={stat} index={i} />)}
                            </div>
                        </div>
                        {mainText && (
                            ...
                            >
                                <div style={{ fontSize: activeStats.length > 2 ? '1.25cqi' : '1.4cqi', color: 'var(--slide-body)', lineHeight: 1.5, fontWeight: 500 }}>
                                    <SlideMarkdown content={mainText} />
                                </div>
                            </motion.div>
                        )}

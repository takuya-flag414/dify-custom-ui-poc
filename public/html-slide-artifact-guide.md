...
スライドサイズは **960px × 540px（16:9）** を基準とする。  
フロントエンドは `isHtmlSlide` 判定時に basePaperWidth = 1020、minHeight = 540 でiframe を描画する。  
`@page { size: 960px 540px landscape; margin: 0; }` は省略禁止とする。
...
- `@page { size: 960px 540px landscape; margin: 0; }` があるか
...
*このガイドライン v1.0 に従うことで、LLM および実装者はスライドの種別・情報密度・グラフの必要性・スライド構成・収容安全性・分割後ランタイムの挙動を総合的に判断し、崩れにくく視覚的に品位あるプレゼンテーション HTML Artifact を安定して生成できる。*
